...
extern uint8_t work_start_once; // 用于控制一次性操作
extern uint8_t work_end_once;
...
INTERRUPT_HANDLER(EXTI_PORTC_IRQHandler, 5)
{
   /* In order to detect unexpected events during development,
      it is recommended to set a breakpoint on the following instruction.
   */
   // enableInterrupts(); // 确保在主程序或初始化部分调用此函数
   ...
   if (GPIO_ReadInputPin(GPIOC, GPIO_PIN_5) == RESET)
   {
      // 按键被确认按下，处理有效的按键操作
      // 处理按键按下事件
      work_flag ^= 1; // 这将把work_flag从0变为1，或从1变为0
      work_end_once = 0;
      work_start_once = 0;
...
INTERRUPT_HANDLER(TIM1_UPD_OVF_TRG_BRK_IRQHandler, 11)
{
   /* In order to detect unexpected events during development,
      it is recommended to set a breakpoint on the following instruction.
   */
   //   TIM1_ClearITPendingBit(TIM1_IT_UPDATE); // 清除定时器中断标志
   ...
   //     TIM1_Cmd(DISABLE); // 停止定时器直到下一个按键中断触发
}
